...
- `analytics` <Type text="object" /> <MetaInfo text="optional" />
  - An account-scoped Analytics SQL binding. Refer to [Analytics SQL](#analytics-sql).
...
The [Analytics SQL binding](/analytics/sql-api/workers-binding/) queries account-scoped analytics datasets without an API token. Assign an object with the following fields to the `analytics` key:

- `binding` <Type text="string" /> <MetaInfo text="required" />
  - The binding name used to query Analytics SQL from your Worker.
- `remote` <Type text="boolean" /> <MetaInfo text="optional" />
  - Set to `true` to acknowledge that local development connects to the remote service and suppress the remote resource warning. Analytics SQL has no local implementation and always uses the remote service.
