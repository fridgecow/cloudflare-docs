...
- `analytics` <Type text="object" /> <MetaInfo text="optional" />
  - An Analytics SQL binding. Refer to [Analytics SQL](#analytics-sql).
...
The [Analytics SQL binding](/analytics/sql-api/workers-binding/) queries analytics datasets without an API token. Assign an object with the following fields to the `analytics` key:
