...
The Analytics SQL binding queries analytics datasets from a Worker. The binding authenticates as the account that owns the Worker, so it does not require an API token.
